...
def send_twitter(APP_NAME,CONSUMER_KEY,CONSUMER_SECRET,tweet):
    ...
    tw = twitter.Twitter(auth= \
                      twitter.OAuth(oauth_token, oauth_secret, \
...
if __name__ == '__main__' :
    ...
    parser.add_argument('-t', '--twitter',dest='is_tweet', action='store_true',default = False)
    parser.add_argument('-ck', '--consumer_key', default=CONSUMER_KEY)
    parser.add_argument('-cs', '--consumer_secret', default=CONSUMER_SECRET)
    parser.add_argument('-m', '--mail',dest='is_mail', action='store_true',default = False)
    parser.add_argument('-u', '--user', default='')
    parser.add_argument('-p', '--password', default='')
    args = parser.parse_args()

    if(args.is_tweet and (args.consumer_key=='' or args.consumer_secret=='')):
        print(em( 'Arg Error required consumer key and consumer secret when you use twitter' ))
        exit(1)
